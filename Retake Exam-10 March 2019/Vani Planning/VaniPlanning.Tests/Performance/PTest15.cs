...
public class PTest15
{
    ...
    public void GetAllInvoiceInPeriod_10000_Elements()
    {
        ...
        var mostOccurrencesIssueDate = this.inputGenerator.MostOccur(invoices, x => x.IssueDate);
        var mostOccurrencesDueDate = this.inputGenerator.MostOccur(invoices, x => x.DueDate);

        var watch = new Stopwatch();
        watch.Start();

        this.agency.GetAllInvoiceInPeriod(mostOccurrencesIssueDate.Key, mostOccurrencesDueDate.Key);
        watch.Stop();

        long elapsedTime = watch.ElapsedMilliseconds;
        Assert.IsTrue(elapsedTime <= 40);
